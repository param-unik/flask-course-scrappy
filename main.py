...
def index():
    if request.method == 'POST':
        # try:
            ...
            try:
                coursesURL = res.html.xpath('/html/body/section[1]/div/div[1]/a', first=True)
            except:
                app.logger.error("Not able to parse web page.") 

            try:  
                for url in coursesURL.absolute_links:
                    print(url)
                    r = sess.get(url)
                    # res.html.render(sleep=1)
                    # allCoursesLink = res.html.xpath('//*[@id="__next"]/div[1]/section[2]/div/div[2]/div', first =True)
                    links = r.html.find('div.TopCategoryList_category-cards__LvHCa TopCategoryList_grid__2Pi2T grid', first = True)
                    print(links)
            except:
